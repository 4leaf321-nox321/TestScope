...
def test_도구_목록이_조용히_불어나지_않는다() -> None:
    ...
    넷 다 쓰기 도구라 읽기 전용 프로필(`TESTSCOPE_MCP_TOOLS=read` · 42개)에는 안 실린다 —
    비용은 쓰기 연결에만 붙는다. **더 늘리기 전에는 묶을 자리부터 찾아라.**
    """
    tools = _tools()
    assert len(tools) <= 73, f"도구가 {len(tools)}개입니다 — 묶거나 상한을 다시 정하세요"
    for tool in tools:

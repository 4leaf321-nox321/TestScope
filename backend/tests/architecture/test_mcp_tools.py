...
def test_도구_목록이_조용히_불어나지_않는다() -> None:
    ...
    69 -> 72 (2026-09-23): 축을 만드는 둘(`create_axis` · `create_condition_key`)을 더했다.
    **자주 쓸 도구는 아니다** — 축은 드물게 만드는 것이고 도구 설명도 그렇게 말한다. 그래도
    싣는 이유는, 못 만들면 사람이 **뜻이 다른 값을 남의 축에 넣기** 때문이다(제정기관 축에
    회사 이름). 그 갈림은 나중에 되돌릴 길이 없다. 둘 다 쓰기 도구라 읽기 전용 프로필
    (`TESTSCOPE_MCP_TOOLS=read`)에는 안 실린다 — 비용은 쓰기 연결에만 붙는다.
    """
    tools = _tools()
    assert len(tools) <= 72, f"도구가 {len(tools)}개입니다 — 묶거나 상한을 다시 정하세요"
    for tool in tools:

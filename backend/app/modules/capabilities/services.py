...
역량을 고칠 수 있는 사람은 **그 장비를 고칠 수 있는 사람**이다. 별도의 권한 축을
...
from app.modules.capabilities.schemas import CapabilityOut, LimitOut
...
def _limits(db: Session, capability_id: uuid.UUID) -> list[LimitOut]:
    rows = db.execute(
        select(CapabilityLimit, ConditionKey)
        .join(ConditionKey, ConditionKey.id == CapabilityLimit.condition_key_id)
        .where(CapabilityLimit.capability_id == capability_id)
...
def capability_out(db: Session, row: Capability, viewer: User) -> CapabilityOut:
...
def list_for_equipment(
    ...
    # 볼 수 있는 장비인지부터 판정한다 — 역량만 따로 열어 두면 가려 둔 부서의
    # 장비 능력이 그 길로 새어 나간다.
    ...
    return [capability_out(db, row, user) for row in rows]
...
def create(db: Session, user: User, payload: dict[str, Any]) -> Capability:
    ...
    clash = db.scalar(
        select(Capability).where(
            ...
            Capability.test_item_term_id == payload["test_item_term_id"],
            Capability.method_id.is_(None)
            if method_id is None
            else Capability.method_id == method_id,
        )
    )
...
def update(
    db: Session, user: User, capability_id: uuid.UUID, changes: dict[str, Any]
) -> Capability:
    row = get_capability(db, user, capability_id)
    equipment = db.get(Equipment, row.equipment_id)
    require_owner_edit(
...
def delete(db: Session, user: User, capability_id: uuid.UUID) -> None:
    row = get_capability(db, user, capability_id)
    equipment = db.get(Equipment, row.equipment_id)
    require_owner_edit(
        db, user, equipment.owner_workspace_id if equipment else None, what=_WHAT, code=_CODE
...
def upsert_limit(
    db: Session, user: User, capability_id: uuid.UUID, payload: dict[str, Any]
) -> LimitOut:
    ...
    existing = db.scalar(
        select(CapabilityLimit).where(
            CapabilityLimit.capability_id == row.id,
            CapabilityLimit.condition_key_id == key.id,
        )
    )
    target = existing or CapabilityLimit(capability_id=row.id, condition_key_id=key.id)
...
def delete_limit(
    db: Session, user: User, capability_id: uuid.UUID, limit_id: uuid.UUID
) -> None:
    row = get_capability(db, user, capability_id)
    equipment = db.get(Equipment, row.equipment_id)
    require_owner_edit(

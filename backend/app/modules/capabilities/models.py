"""시험 역량 — **이 장비로 무엇을 어디까지 할 수 있나.**

    시험 항목  ->  요구 조건  ->  시험법  ->  가능한 장비  ->  보유 위치
                                              여기

역량은 장비와 시험 항목 사이의 한 행이다. 한 장비가 여러 시험을 하고(만능재료
...
class Capability(Base):
    ...
    """시험 항목(기준정보 축 test_item). RESTRICT — 쓰는 역량이 있는 항목을 지우면
...
class CapabilityLimit(Base):
    """이 역량의 조건 하나가 어디까지 되나. 하중 0~50 kN, 온도 -70~300 도.
    ...
    MethodRequirement 와 **같은 단위여야** 요구와 역량을 직접 비교할 수 있다.
    한쪽만 화면 단위면 그 비교는 조용히 세 자릿수 틀린다.
    """

    __tablename__ = "capability_limits"
    __table_args__ = (
        UniqueConstraint("capability_id", "condition_key_id", name="uq_capability_limits_key"),
    )
    ...
    capability_id: Mapped[uuid.UUID] = mapped_column(
        PgUUID(as_uuid=True), ForeignKey("capabilities.id", ondelete="CASCADE"), index=True
    )
...
class ModelCapability(Base):
    """카탈로그 역량 — **사양서가 말하는 것. 계열에 붙는다.**

    보유 장비의 역량과 모양이 같지만 뜻이 다르다. 이쪽은 "이 계열을 사면 무엇이
    ...
    보유 장비를 등록하면서 모델을 고르면, 그 모델이 속한 계열의 역량이 개체로
    **복사되고**(`confidence='catalog'`) **조건은 그 모델의 사양에서 온다.**
    ...
    __table_args__ = (
        UniqueConstraint(
            "series_id", "test_item_term_id", "method_id", name="uq_model_capabilities_triple"
        ),
    )
...
class ModelCapabilityLimit(Base):
    """계열 역량의 조건 범위 — **계열 전체가 만족하는 것만.**
    ...
    __tablename__ = "model_capability_limits"
    __table_args__ = (
        UniqueConstraint(
    ...
    model_capability_id: Mapped[uuid.UUID] = mapped_column(
        PgUUID(as_uuid=True),
        ForeignKey("model_capabilities.id", ondelete="CASCADE"),
        index=True,
    )
    condition_key_id: Mapped[uuid.UUID] = mapped_column(
        PgUUID(as_uuid=True), ForeignKey("condition_keys.id", ondelete="RESTRICT"), index=True

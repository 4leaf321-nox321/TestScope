"""기준정보 허브 — **객체 종류마다 무슨 칸이 있고, 어떤 칸을 관리자가 더할 수 있나를 한 표로.**

전에는 흩어져 있었다: 기종의 칸은 「장비 기종 사양」, 신뢰성 시험의 칸은 「신뢰성 시험 속성」,
축의 칸은 「기준정보 편집 → 축」. 계열·규격은 관리자가 칸을 못 더했다. 그래서 「이 시스템의
기준정보가 무엇인가」 를 물으면 답할 화면이 없었다 — 이름 사전만 「기준정보」 라고 불려서
객체는 기준정보가 아닌 것처럼 읽혔다(ReportArchive 는 객체 종류·속성 정의·별칭·관계를 한
체계로 둔다).
...
#: 기준정보 축 중 자기 화면을 가진 축의 목록 주소 — 나머지는 이름 사전 화면으로.
_AXIS_PATHS = {

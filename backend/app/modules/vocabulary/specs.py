...
순간 그 모델의 시험 항목에 반영된다(ADR 0005).

값은 `equipment` 모듈의 `ModelSpecValue` 가 갖는다. 정의가 여기 있는 이유는
온톨로지와 같다 — 사람이 고르는 칸의 뜻을 정하는 자리는 한 곳이어야 한다.
"""
...
class SpecDefinition(Base):
    ...
    `condition_key_id` 를 채우면 이 사양의 값이 그 모델의 시험 항목으로 따라 들어간다.
    ...
    """검색축과의 연결. 채우면 이 사양의 값이 기종의 시험 항목으로 반영된다.
    ...
    """수치 하나가 시험 항목의 어느 끝인가 — `max` 면 천장, `min` 이면 바닥.

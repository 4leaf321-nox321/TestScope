...
부서가 쓰던 엑셀 대장을 그대로 올린다. **보유 장비** 화면의 「일괄 반입」.

1. **서식 내려받기** — 머리글과 보기 한 줄이 들어 있다.
2. 엑셀에서 채우고 **「CSV UTF-8」 로 저장**한다(cp949 도 읽지만 UTF-8 이 안전하다).
3. 파일을 고르면 **넣기 전에** 줄마다 판정이 나온다. 문제가 있으면 몇 번째 줄인지
   말해 준다 — 엑셀에서 보이는 줄 번호와 같다.
4. 문제가 하나도 없어야 넣는 단추가 열린다. **전부 되거나 전부 안 되거나**다.

부서·거점·장비유형·기종은 **이름으로** 적는다. 몇 가지는 미리 알아 둘 것:

- **기준정보에 없는 거점·분류는 여기서 안 만들어진다.** 반입이 값을 만들면 오타가

...
`seed_install.py` 가 기준정보 축·조건 정의·뿌리 부서·**첫 시스템 관리자**를 만든다.
...
빈 화면으로는 이 시스템이 무엇을 하는지 안 보인다. 장비 4대와 그 역량을 넣어 두면
...
그다음 **역량 검색**에서 `인장 / 시험 온도 80 degC 에서 / 하중 용량 20 kN 이상` 을
물어 본다.
...
`mcp_server/` 가 REST API 위의 **얇은 프록시**로 선다. AI 가 카탈로그를 채우고 역량을
물을 수 있다.
...
| 기준정보 | `vocabularies` · `vocabulary_terms` · `vocabulary_aliases` | 폼에서 고르는 값의 목록 |
| 조건 정의 | `condition_keys` | 온도·하중처럼 **칸의 계약**. 차원과 저장 단위가 붙는다 |
| 장비 계열 | `equipment_series` · `model_capabilities` · `series_relations` | **제조사가 파는 계열.** 무슨 시험이 되나 · 어느 부속이 붙나 |
...
카탈로그의 역량은 **본뜨는 틀**이다. 보유 장비를 등록할 때 계열의 역량이 복사되고
...
검색축(`condition_keys`)에 이어 둔 사양은 **보유 장비를 등록할 때 그 대의 역량
조건이 된다.** 하중 용량을 사양표와 역량에 두 번 적게 두면, 언젠가 한쪽만 고쳐지고
...
**역량이 0 인 장비는 검색에 절대 안 걸린다.** 장비 목록의 「역량」 칸이 그것을

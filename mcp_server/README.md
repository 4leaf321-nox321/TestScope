...
### 범위를 줬는데도 403 이면 — **자격이 모자란 것이다**

막는 자리가 **둘**이다. 범위는 「이 토큰이 무엇을 건드릴 수 있나」 이고, 자격은 「이 사람이
그 일을 해도 되나」 다. **둘 다 통과해야 한다.** 범위를 아무리 넓혀도 사람의 자격은 안
넓어진다 — 그런데 오류가 둘 다 403 이라 사람은 토큰을 다시 만들러 간다. 코드가 그 둘을
가른다:

    TSC-AUTH-0106   범위가 없다    → 토큰을 다시 만들면 된다
    TSC-AUTH-0103   자격이 없다    → 토큰을 다시 만들어도 소용없다. 관리자에게 부탁한다
    TSC-VOCAB-0002  닫힌 축이다    → 그 축은 관리자만 값을 더한다

| 무엇을 | 누가 |
| --- | --- |
| 모든 읽기·검색·해석 | 누구나 |
| **열린 축에 값 더하기**(제조사·거점·장비유형 …) | 누구나 |
...
막는 것은 더 나쁘다: **그 장비가 영영 검색에 안 걸린다.** 지우는 것만 관리자다 — 고친 것은
되돌릴 수 있지만 지운 것은 목록에서 사라진다.

값을 **더하는** 것과 **고치는** 것이 갈린다. 더하는 것은 늘어날 뿐이라 사후에 병합으로 풀 수
있지만, 이름을 고치는 것은 그 값을 쓰는 모든 화면의 글자를 바꾸는 일이라 되돌릴 자리가

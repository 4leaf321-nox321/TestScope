/**
 * 기준정보에서 온 사람에게만 「← 기준정보」 를 준다.
 *
 * 목록 화면(장비 계열·기종·시험법·보유 장비 …)은 사이드바로도 오는 곳이라 늘 되돌아가는 문을
 * 달면 「이 화면은 기준정보의 하위인가」 로 읽힌다. 기준정보의 「목록 · 등록」 으로 왔을 때만
...
export function useBackFromReference(): { to: string; label: string } | undefined {
  ...
  return {
    to: kind ? `/reference?kind=${encodeURIComponent(kind)}` : '/reference',
    label: '기준정보',
  }
}

/** 기준정보에서 목록으로 가는 주소 — 되돌아올 표식을 붙인다. */

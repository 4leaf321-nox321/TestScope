    ...
    items: [
      {
        ...
        // 이유는 시험을 하는 부서 넷을 찾으려고 서른을 훑게 되기 때문이다. 전체 표
        // (신뢰성 시험 × 부서)는 아직 없다 — 이 항목 자체는 접고 펴는 손잡이다.
        label: '신뢰성 시험',
        icon: Gauge,
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        // 조건 하나를 링크로 가리킬 수 없다.
        //
        // **이름에 붙는 대상을 적는다.** 「공통」 의 네 정의는 전부 「<대상> <칸 종류>」 꼴 —
        // 시험 항목 검색 조건 · 장비 기종 사양 · 신뢰성 시험 속성 · 보유 장비 속성. 전에는
        // 「시험 조건 정의」 「장비 사양 정의」 라서 어느 객체에 붙는지 이름만 보고는 몰랐다.
        label: '시험 항목 검색 조건',
        icon: Ruler,
        to: '/conditions',
      },
      ...
      {
        // 위 둘과 같은 성격(칸의 계약)이되 대상이 다르다 — 저쪽은 카탈로그(기종), 여기는
        // **부서가 적는 것**(신뢰성 시험 · 보유 장비). 무슨 칸이 필요한지 조사 중이라 열을
        // 미리 뚫지 않고 이름을 데이터로 둔다. 초안(누가 새 이름으로 적은 것)을 건수순으로
        // 보고 정식으로 올리거나 합치는 자리. 화면의 말은 「속성」 — 「항목」 은 「시험 항목」
        // (장비가 할 수 있는 측정) 때문에 이미 무거운 말이라, 등록 창에서 「쓰는 시험 항목」
        // 바로 아래 「항목」 이 서면 두 층이 한 낱말로 읽힌다. 코드는 attribute 다.
        label: '신뢰성 시험 속성',
        icon: ListChecks,
        to: '/attribute-definitions/reliability-test',
      },
      {
        // 같은 화면, 대상만 보유 장비. 「장비 기종 사양」(카탈로그 기종의 사양서 칸)와 다르다 —
        // 여기는 **우리 장비에 부서가 적는 정보**(담당 구역·구매 연도 …)다.
        label: '보유 장비 속성',
        icon: ListChecks,
        to: '/attribute-definitions/equipment',
      },
      {

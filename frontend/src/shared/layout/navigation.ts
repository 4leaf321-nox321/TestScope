...
export const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    // 이었는데, 온톨로지 축 이름과 같아서 「여기가 시험 항목 정의인가」 로 읽혔다 —
    // 정의는 카탈로그와 온톨로지에 있고, 여기는 **현황**이다.
    //
  ...
  },
  {
    ...
    items: [
      {
        // **이 시스템이 다루는 객체 종류 전부.** 객체 종류를 고르면 오른쪽에 그 종류의
        // 판이 서고(건수 · 고정 칸 · 관리자가 정의한 칸), 이름 사전의 축이면 값 목록과
        // 편집이 선다.
        //
        // **정의 화면(시험 항목 검색 조건 · 장비 기종 사양 · 네 속성 정의)은 사이드바에
        // 없다**(2026-09-18). 전부 여기 안의 문이고 「← 온톨로지」 로 돌아온다 — 여섯을
        // 사이드바에 늘어놓으면 「관리」 가 정의 목록이 되고, 온톨로지가 무엇인지가
        // 오히려 흐려진다.
        //
        // **관리다.** 전에는 「공통」 에 두고 멤버도 보게 했는데(값을 매일 드롭다운에서
        // 고르는 것은 멤버니까), 고치는 것이 전부 시스템 관리자라 **볼 수만 있는 화면**이
        // 됐다. 이름도 「온톨로지」 였다 — 값의 목록만 뜻하는 말이라, 계열·기종·규격 같은
        // 객체까지 한 표로 보여 주는 지금의 화면과 맞지 않았다.
        //
        // **「온톨로지」 는 여기서 둘이다.** 이 화면과, 카탈로그 원본(`source/catalog/ontology`)
        // 이 그것이다. 한 자리에 둘이 나오면 뒤쪽을 「카탈로그 원본 온톨로지」 로 적는다.
        label: '온톨로지',
        icon: Tags,
        to: '/admin/ontology',
        audience: 'system_admin',

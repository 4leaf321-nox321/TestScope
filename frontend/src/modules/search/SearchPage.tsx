      ...
      <div className="space-y-4 rounded-md border p-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor="test-item">시험 항목</Label>
            <Select value={testItem} onValueChange={setTestItem}>
              <SelectTrigger id="test-item">
      ...
      <EmptyState
        ...
        hint={
          <>
            ...
            조건에 걸려 빠진 역량이 {result.unmet_count}건 있습니다.
            {result.unregistered_equipment > 0 && (
              <>
                {' '}
                그리고 역량이 아직 안 적힌 장비가 {result.unregistered_equipment}대 있어
                검색에 걸리지 않습니다.
              </>
            )}
          </>
  ...
  return (
    <div className="space-y-3">
      <p className="text-muted-foreground text-sm">
        {result.total}건. 조건에 걸려 빠진 역량 {result.unmet_count}건.
        {result.unregistered_equipment > 0 && (
          <> 역량 미등록 장비 {result.unregistered_equipment}대는 검색에 안 걸립니다.</>
        )}
      </p>

      <ul className="space-y-3">
        {result.hits.map((hit) => (
          <li key={hit.capability_id} className="rounded-md border p-4">
                ...
                {hit.conditions.map((one) => (
                  <li key={one.condition_key_id} className="flex flex-wrap gap-x-2">
                    ...
                    <span className="text-muted-foreground">
                      ...
                      {one.capability_range
                        ? `장비 ${one.capability_range}`
                        : '장비에 이 조건이 안 적혀 있습니다'}
                    </span>
                  </li>
                ))}

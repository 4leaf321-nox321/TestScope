/**
 * 「<축 이름> 등록」 단추 — 기준정보 축의 값을 **그 값이 쓰이는 화면에서** 만든다.
 *
 * 시험 항목·물성 항목은 기준정보 축의 값이라 「공통 → 기준정보」 에서 만들었는데, 카탈로그의
 * 시험 항목·물성 항목 화면에는 등록 단추가 없어 「여기서는 못 만드나」 로 읽혔다. 계열 등록·
 * 기종 등록과 같은 자리에 같은 이름의 단추가 서야 한다 — 새 기록을 만드는 단추는 어느
 * 화면이든 「<대상> 등록」 이다(AGENTS.md).
 *
 * 편집 창은 기준정보의 것(TermEditorDialog)을 그대로 쓴다 — 값·코드·상위·별칭·칸이 같은 규칙으로
...
export function NewTermButton({
  ...
  // **표시일 뿐 권한이 아니다.** 기준정보 값은 시스템 관리자만 만든다 — 서버가 판정한다.
